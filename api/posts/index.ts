...
export default async function handler(
  ...
  if (req.method === 'POST') {
    ...
    if (!Array.isArray(mediaUrls) || mediaUrls.length === 0 || mediaUrls.length > 10) {
      return res.status(400).json({ error: 'mediaUrls must be 1-10 items' });
    }
    if (!mediaUrls.every((u) => typeof u === 'string' && u.length > 0 && u.length < 5_000_000)) {
      return res.status(400).json({ error: 'Invalid mediaUrls' });
    }
    if (!isNonEmptyString(title) || title.length > 100) {
      return res.status(400).json({ error: 'Invalid title' });
    }
    if (!isNonEmptyString(content) || content.length > 5000) {
